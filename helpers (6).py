...
def clean_data(data):
    ...
    #feature column 22 contains integer values:
    data[inds, 22] = np.take(np.floor(means), inds[1])
    return data
...
def standardize_test(x, mean, std):
    #Standardize the test data set with training set mean and standard deviation:
    x = (x - mean) / std
    return x
